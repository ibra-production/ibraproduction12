    ...
    {tab==='crm'&&<div className="space-y-4">
      ...
      {selected&&<div className="fixed inset-0 z-[120] bg-black/80 p-3 md:p-6 overflow-auto"><div className="max-w-6xl mx-auto bg-neutral-900 border border-neutral-700 rounded-3xl shadow-2xl overflow-hidden">
        <div className="p-5 md:p-7 bg-gradient-to-br from-neutral-950 to-neutral-900 border-b border-neutral-800">
          <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs text-amber-400 font-black tracking-widest">IBRA PRODUCTION • CLIENT FILE</div><h3 className="text-3xl font-black mt-2">{selected.groomName||selected.fullName||'Client'} {selected.brideName?'× '+selected.brideName:''}</h3><div className="text-sm text-neutral-400 mt-2">{selected.phone||'—'} • {selected.email||'—'}</div><div className="flex flex-wrap gap-2 mt-3"><span className="px-3 py-1 rounded-full bg-amber-500/10 text-amber-300 font-mono text-xs">{selected.clientCode||selected.barcode||'NO-CODE'}</span><span className="px-3 py-1 rounded-full bg-neutral-800 text-neutral-300 text-xs">{selected.bookings?.length||0} حجز</span></div></div><div className="flex gap-2"><button onClick={()=>printClientCard(selected)} className="px-4 py-2 bg-amber-500 text-black rounded-xl font-black"><Printer className="w-4 h-4 inline ml-1"/> ID Card</button><button onClick={()=>setSelected(null)} className="px-3 py-2 bg-neutral-800 rounded-xl">إغلاق</button></div></div>
        </div>
        ...
        <div className="p-5 md:p-7">
          ...
          {clientTab==='activity'&&<div className="space-y-2">{scanLogs.filter(l=>l.clientId===selected.id||l.clientCode===selected.clientCode).slice().reverse().map(l=><div key={l.id} className="bg-neutral-950 rounded-xl p-4 text-sm"><span className={l.success?'text-emerald-400':'text-red-400'}>{l.success?'SCAN ناجح':'SCAN فاشل'}</span> • <span className="font-mono">{l.code}</span></div>)}{(selected.bookings||[]).map((b:any)=><div key={'b-'+b.id} className="bg-neutral-950 rounded-xl p-4 text-sm">حجز #{b.id} • {dates(b).join(' • ')} • {b.status}</div>)}</div>}
        </div>
      </div></div>}
    </div>}
